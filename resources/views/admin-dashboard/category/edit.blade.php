...
<div class="container border px-5 pt-5 w-1/2 rounded-md text-white mx-auto">
    ...
    <form action="{{ url('/admin-dashboard/category/'. $category->code_category) }}" class="flex flex-col my-5 px-3 space-y-3" method="post" enctype="multipart/form-data">
        @csrf
        ...
        <div class="flex flex-col space-y-3">
            ...
            <textarea name="description" rows="5" class="text-black rounded-md p-3 @error('description') border border-red-500 @enderror">{{ $category->description }}</textarea>
            @error('description')
                <p class="text-xs text-red-500">{{ $message }}</p>
            @enderror
        </div>
